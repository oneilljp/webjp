...
export function dbfs(start, end, board, dfs) {
    ...
    if (!legalPosition(start.row, start.col, board) || !legalPosition(end.row, end.col, board)) {
    ...
    function s() {
        if (deque.length == 0) {
            ...
            try {
                ...
                setTimeout(checkPos(current.row - 1, current.col, board, memo, deque, 's', dfs, end), 1000);
                setTimeout(checkPos(current.row, current.col + 1, board, memo, deque, 'w', dfs, end), 2000);
                setTimeout(checkPos(current.row + 1, current.col, board, memo, deque, 'n', dfs, end), 3000);
                setTimeout(checkPos(current.row, current.col - 1, board, memo, deque, 'e', dfs, end), 4000);
            }
            catch (e) {
