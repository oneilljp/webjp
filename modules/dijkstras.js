...
function addPos(memo, board, locations, current, nextPos, end) {
    if (!memo[nextPos.row][nextPos.col].discovered || memo[nextPos.row][nextPos.col].distance > memo[current.row][current.col].distance + 1) {
        memo[nextPos.row][nextPos.col].discovered = true;
        memo[nextPos.row][nextPos.col].distance = memo[current.row][current.col].distance + 1;
